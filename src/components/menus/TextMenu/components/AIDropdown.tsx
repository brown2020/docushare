...
export const AIDropdown = ({
  ...
  const performAiCommand = (action:() => void) => (event: React.MouseEvent) => {
  ...
  return (
    <Root open={open} onOpenChange={(_open) => { if (_open) { 
      onDiscardAiContent(); 
      setAiContinue('');
      } setOpen(_open) }}>
      <Trigger asChild>
      ...
      <Content asChild align='start'>
        <Surface className="p-2 min-w-[10rem]">
          {
            aiProcessing ? (<div className='outline-none' onClick={(event) => event.preventDefault()}>
              <DropdownButton>
            ...
            </div>) : aiContent ? <Fragment >
              <div className='outline-none px-2 py-1 max-h-[60vh] overflow-y-auto' onClick={(event) => event.preventDefault()}>
                <Markdown className='max-w-[70vw] prose prose-sm prose-neutral'>{aiContent}</Markdown>
              </div>
              <hr className='my-2' />
              ...
              <div onKeyDown={(e) => e.stopPropagation()} className='flex items-center gap-2'>
                  <Icon name="Sparkles" />
                  <input onKeyDown={handleKeyDown} onChange={(event) => {
                    event.stopPropagation();
                    event.preventDefault(); setAiContinue(event.target.value)}} value={aiContinue} className='p-0 outline-none border-0 focus:ring-0' placeholder='Ask AI to edit or generate...' />
                  <Button onClick={() => performAiCommand(handleCustomAiCommand())} variant='icon' className='ml-2'>
                    <Icon name="CircleChevronUp" />
                  </Button>
              </div>
              {
                aiCommands.filter((command) => aiContinue ? command.label.toLowerCase().includes(aiContinue.toLowerCase()) : true).map((command, index) => <div key={index} onClick={performAiCommand(command.command)}>
                <DropdownButton>
