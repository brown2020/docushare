...
export const AIDropdown = ({
  ...
  return (
    <Popover.Root open={open} onOpenChange={(_open) => {
      if (_open) {
        onDiscardAiContent();
        setAiContinue('');
      } setOpen(_open)
              ...
              aiProcessing ? (<div className='outline-none' onClick={(event) => event.preventDefault()}>
                ...
                <div onKeyDown={(e) => e.stopPropagation()} className='flex items-center gap-2'>
                  ...
                  <input onKeyDown={handleKeyDown} onChange={(event) => {
                    ...
                    event.preventDefault(); setAiContinue(event.target.value)
                  }} value={aiContinue} className='p-0 outline-none border-0 focus:ring-0' placeholder='Ask AI to edit or generate...' />
