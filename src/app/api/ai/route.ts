...
import { createOpenAI, openai as openai1 } from '@ai-sdk/openai'; // Ensure OPENAI_API_KEY environment variable is set
import { NextRequest } from "next/server";
import { db } from "@/firebase/firebaseAdminConfig";
import { AI_MODEL_CLAUDE, AI_MODEL_GEMINI, AI_MODEL_GPT, AI_MODEL_LLAMA, AI_MODEL_MISTRAL, AI_MODELS_LIST, DEFAULT_AI_MODEL, RESPONSE_CODE, USER_COLLECTION } from "@/lib/constants";
...
async function getModel(modelName: string, api_key: string) {
  switch (modelName) {
    ...
    case AI_MODEL_GEMINI:
      return createGoogleGenerativeAI({apiKey: api_key})("models/gemini-1.5-pro-latest")
...
async function getModelForUser(user_id: string) {
  const userRef = db.collection(USER_COLLECTION).doc(user_id).collection("profile").doc('userData');
  ...
  if (availableModelKeys.length == 0) return { status: false, code: RESPONSE_CODE.api_key_not_set }

  const aiModelKeys: Record<string, string> = {
    [AI_MODEL_GPT]: data && data.openai_api_key ? data.openai_api_key : '',
    [AI_MODEL_GEMINI]: data && data.anthropic_api_key ? data.anthropic_api_key : '',
    [AI_MODEL_MISTRAL]: data && data.google_gen_ai_api_key ? data.google_gen_ai_api_key : '',
  ...
  if (Object.keys(data).includes('ai_model') && typeof data.ai_model == 'string' && AI_MODELS_LIST.includes(data.ai_model)) {
    // if user has selected ai model then use only that
    if (!availableModelKeys.includes(data.ai_model)) return { status: false, code: RESPONSE_CODE.api_key_not_found }
  }

  // If not selected then use first default model
  if (availableModelKeys.includes(DEFAULT_AI_MODEL)) return { status: true, data: { model: DEFAULT_AI_MODEL, key: aiModelKeys[DEFAULT_AI_MODEL] } }
...
export const POST = async (req: NextRequest) => {
  ...
  if (companyOpenAIKey.length == 0) {
    return new Response(JSON.stringify({ status: false, message: 'Something went wrong.', code: "ai_key_not_found" }))
  }
  ...
  if(!modelOfUser.status || !modelOfUser.data){
    return new Response(JSON.stringify({ status: false, message: "You have not added any credit or AI key.", code: modelOfUser.code }));
  }
